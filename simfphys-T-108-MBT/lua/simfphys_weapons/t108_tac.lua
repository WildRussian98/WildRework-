...
simfphys.weapon.M240Clipsize = 165

local function hmg_fire(ply,vehicle,shootOrigin,shootDirection)

	vehicle:EmitSound("simulated_vehicles/weapons/DSHK.ogg")
...
function simfphys.weapon:ValidClasses()
	return { "sim_fphys_tank_t108_tac" }
end

function simfphys.weapon:Initialize( vehicle )
	net.Start( "avx_misc_register_tank" )
		net.WriteEntity( vehicle )
		net.WriteString( "t108_tac" )
	net.Broadcast()
	
	vehicle:SetNWBool( "SpecialCam_Loader", true )
	vehicle:SetNWFloat( "SpecialCam_LoaderTime", 11 )
	


	simfphys.RegisterCrosshair( vehicle:GetDriverSeat(), { Attachment = "cannon_muzzle", Type = 2 } )
	simfphys.RegisterCamera( vehicle:GetDriverSeat(), Vector(30,22,5), Vector(0,60,120), true, "cannon_mg_muzzle" )
	...
	simfphys.RegisterCamera( vehicle.pSeat[2], Vector(36,-58,45), Vector(0,60,120), true, "cannon_mg_muzzle" )
...
function simfphys.weapon:AimCannon( ply, vehicle, pod, Attachment )	
	...
	local pclamp = math.Clamp( (math.cos( math.rad(TargetAng.y + 180) ) - 0.465) * 3,0,1) ^ 2 * 7
	vehicle:SetPoseParameter("cannon_aim_pitch", math.Clamp(-TargetAng.p,-9 + pclamp,40) )
end
...
function simfphys.weapon:UpdateSuspension( vehicle )
	...
	for i, v in pairs( nk_tank_susdata ) do
		local pos = vehicle:GetAttachment( vehicle:LookupAttachment( nk_tank_susdata[i].attachment ) ).Pos + Up * -2
		...
		vehicle:SetPoseParameter(nk_tank_susdata[i].poseparameter, vehicle.oldDist[i] )
	end
end
