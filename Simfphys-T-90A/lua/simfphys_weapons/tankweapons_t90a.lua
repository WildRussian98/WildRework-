...
function simfphys.weapon:ValidClasses()
	return { "sim_fphys_tank_t90a" }
end

function simfphys.weapon:Initialize( vehicle )
	net.Start( "avx_misc_register_tank" )
		net.WriteEntity( vehicle )
		net.WriteString( "t90a" )
	net.Broadcast()
	...
	simfphys.RegisterCamera( vehicle:GetDriverSeat(), Vector(5,-30,9), Vector(0,50,110), true, "cannon_mg_muzzle" )
	
	if not istable( vehicle.PassengerSeats ) or not istable( vehicle.pSeat ) then return end

	simfphys.RegisterCrosshair( vehicle.pSeat[1] , { Attachment = "mg_muzzle", Type = 5 } )
	simfphys.RegisterCamera( vehicle.pSeat[1], Vector(-80,-15,7), Vector(0,50,60), true, "mg_muzzle" )
...
function simfphys.weapon:AimCannon( ply, vehicle, pod, Attachment )	
	...
	local pclamp = math.Clamp( (math.cos( math.rad(TargetAng.y) ) - 0.55) * 4,0,1) ^ 1 * 7
	vehicle:SetPoseParameter("cannon_aim_pitch", math.Clamp(-TargetAng.p,-8 + pclamp,20) )
end
