...
simfphys.weapon.ATGMClipsize = 4
simfphys.weapon.GunClipsize = 50

local function cannon_fire(ply,vehicle,shootOrigin,shootDirection)
	vehicle:EmitSound("simulated_vehicles/weapons/autocanon/Close_Distance_2.ogg", 120)
...
local function rocket_fire(ply,vehicle,shootOrigin,shootDirection)
	vehicle:EmitSound( "simulated_vehicles/weapons/Missile_2.ogg" )
	vehicle:EmitSound( "simulated_vehicles/weapons/Rocket_FlyBy.ogg" )
...
function simfphys.weapon:ValidClasses()
	return { "sim_fphys_tank_crysis2_icv" }
end
...
function simfphys.weapon:Initialize( vehicle )
	...
	local data = {}
	data.Attachment = "rocket_muzzle_3"
	data.Direction = Vector(1,0,0)
	data.Attach_Start = "rocket_muzzle_3"
	data.Type = 3

	simfphys.RegisterCrosshair( vehicle:GetDriverSeat(), { Attachment = "turret_muzzle", Direction = Vector(0, 0, -1), Type = 4 } )
	simfphys.RegisterCamera( vehicle:GetDriverSeat(), Vector(39,-76,-2), Vector(0,45,100), true, "rocket_muzzle_3" )
	
	simfphys.RegisterCamera( vehicle.pSeat[1], Vector(-22,-55,24), Vector(0,45,100), true, "rocket_muzzle_3" )
...
function simfphys.weapon:ControlTurret( vehicle, deltapos )
	...
	local ID = vehicle:LookupAttachment( "turret_muzzle" )
...
function simfphys.weapon:PrimaryAttack( vehicle, ply, shootOrigin, Attachment )
   ...
   self.GunClip = self.GunClip or self.GunClipsize
	...
	if self.GunClip <= 0 then
		...
		vehicle:EmitSound("simulated_vehicles/weapons/apc_reload.ogg")
	...
	util.Effect( "arctic_icv_muzzle", effectdata, true, true )
...
function simfphys.weapon:SecondaryAttack( vehicle, ply, deltapos, cPos, cAng )
	...
	if self.ATGMClip <= 0 then
		...
		vehicle:EmitSound("thor/reload/reload.ogg")
	...
	local AttachmentID = vehicle:LookupAttachment( "rocket_muzzle_" .. tostring(vehicle.barrelIndex) )
...
function simfphys.weapon:UpdateSuspension( vehicle )
	...
	for i, v in pairs( icv_susdata ) do
		local pos = vehicle:GetAttachment( vehicle:LookupAttachment( icv_susdata[i].attachment ) ).Pos + Up * -2
		...
		vehicle:SetPoseParameter(icv_susdata[i].poseparameter, vehicle.oldDist[i] )
	end
end

function simfphys.weapon:DoWheelSpin( vehicle )
	local spin_r = (vehicle.VehicleData[ "spin_4" ] + vehicle.VehicleData[ "spin_6" ]) * 0.7
	local spin_l = (vehicle.VehicleData[ "spin_3" ] + vehicle.VehicleData[ "spin_5" ]) * 0.7
