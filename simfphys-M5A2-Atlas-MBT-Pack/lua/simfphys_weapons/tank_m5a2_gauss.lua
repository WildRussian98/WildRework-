local m5a2_gauss_susdata = {}
for i = 1,6 do
	m5a2_gauss_susdata[i] = { 
...
local function cannon_fire(ply,vehicle,shootOrigin,shootDirection)
	...
	timer.Simple( 10, function() 
		if IsValid( vehicle ) then 
			vehicle:EmitSound("gauss_reload")
		end
	end)
	...
	vehicle:GetPhysicsObject():ApplyForceOffset( -shootDirection * 700000, shootOrigin ) 
	...
	simfphys.FireTankGaussProjectile( projectile )
end

function simfphys.weapon:ValidClasses()
	return { "sim_fphys_m5a2_gauss" }
end

function simfphys.weapon:Initialize( vehicle )
	net.Start( "avx_misc_register_tank" )
		net.WriteEntity( vehicle )
		net.WriteString( "m5a2_gauss" )
	net.Broadcast()
	
	vehicle:SetNWBool( "SpecialCam_Loader", true )
	vehicle:SetNWFloat( "SpecialCam_LoaderTime", 8 )
	


	simfphys.RegisterCrosshair( vehicle:GetDriverSeat(), { Attachment = "cannon_muzzle", Type = 4 } )
	simfphys.RegisterCamera( vehicle:GetDriverSeat(), Vector(-30,25,5), Vector(0,60,120), true, "mg_muzzle" )
...
function simfphys.weapon:PrimaryAttack( vehicle, ply, shootOrigin, Attachment )
	...
	local effectdata = EffectData()
		effectdata:SetEntity( vehicle )
	util.Effect( "StriderMuzzleFlash", effectdata, true, true )
...
function simfphys.weapon:UpdateSuspension( vehicle )
	...
	for i, v in pairs( m5a2_gauss_susdata ) do
		local pos = vehicle:GetAttachment( vehicle:LookupAttachment( m5a2_gauss_susdata[i].attachment ) ).Pos + Up * -4
		...
		vehicle:SetPoseParameter(m5a2_gauss_susdata[i].poseparameter, vehicle.oldDist[i] )
	end
end
