...
function simfphys.weapon:ValidClasses()
	return { "sim_fphys_tank_m1a2" }
end

function simfphys.weapon:Initialize( vehicle )
	net.Start( "avx_misc_register_tank" )
		net.WriteEntity( vehicle )
		net.WriteString( "m1a2_abrams" )
	net.Broadcast()
	...
	simfphys.RegisterCamera( vehicle:GetDriverSeat(), Vector(-16,-5,-38), Vector(-25,50,110), true, "cannon_mg_muzzle" )
	
	if not istable( vehicle.PassengerSeats ) or not istable( vehicle.pSeat ) then return end

	simfphys.RegisterCrosshair( vehicle.pSeat[1] , { Attachment = "mg_muzzle", Type = 5 } )
	simfphys.RegisterCamera( vehicle.pSeat[1], Vector(-80,-3,20), Vector(0,0,60), true, "mg_muzzle" )
	
	simfphys.RegisterCamera( vehicle.pSeat[2], Vector(-25,5,55), Vector(-25,5,55), false )
...
function simfphys.weapon:AimCannon( ply, vehicle, pod, Attachment )	
	...
	vehicle:SetPoseParameter("cannon_aim_pitch", math.Clamp(-TargetAng.p,-10 + pclamp,20) )
end
...
function simfphys.weapon:UpdateSuspension( vehicle )
	...
	for i, v in pairs( leopard_susdata ) do
		local pos = vehicle:GetAttachment( vehicle:LookupAttachment( leopard_susdata[i].attachment ) ).Pos + Up * 12
