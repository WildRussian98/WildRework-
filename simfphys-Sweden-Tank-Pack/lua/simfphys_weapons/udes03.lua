...
local function cannon_fire(ply,vehicle,shootOrigin,shootDirection)
	...
	local projectile = {}
		...
		projectile.Size = 10.5
		projectile.DeflectAng = 10
		projectile.BlastRadius = 105
...
function simfphys.weapon:ValidClasses()
	return { "sim_fphys_tank_udes03" }
end

function simfphys.weapon:Initialize( vehicle )
	net.Start( "avx_misc_register_tank" )
		net.WriteEntity( vehicle )
		net.WriteString( "udes03" )
	net.Broadcast()
	
	vehicle:SetNWBool( "simfphys_NoRacingHud", true )
	vehicle:SetNWBool( "simfphys_NoHud", false ) 
	
	vehicle:SetNWBool( "SpecialCam_Loader", true )
	vehicle:SetNWFloat( "SpecialCam_LoaderTime", 11.7 )

	simfphys.RegisterCrosshair( vehicle:GetDriverSeat(), { Attachment = "cannon_muzzle", Type = 2 } )
	simfphys.RegisterCamera( vehicle:GetDriverSeat(), Vector(8,-88,12), Vector(30,-20,80), true, "can_mg_muzzle" )
	
	if not istable( vehicle.PassengerSeats ) or not istable( vehicle.pSeat ) then return end

	simfphys.RegisterCrosshair( vehicle.pSeat[1] , { Attachment = "mg_muzzle", Type = 1 } )
	simfphys.RegisterCamera( vehicle.pSeat[1], Vector(-70,0,10), Vector(0,0,60), true, "mg_muzzle" )
...
function simfphys.weapon:AimMachinegun( ply, vehicle, pod )	
	...
	vehicle:SetPoseParameter("mg_aim_yaw", TargetAng.y - 90 )
...
function simfphys.weapon:AimCannon( ply, vehicle, pod, Attachment )	
	...
	local TargetAng = Angle(vehicle.sm_pp_pitch,0)
	TargetAng:Normalize() 
...
function simfphys.weapon:AimBody( ply, vehicle, pod, Attachment )	
	if not IsValid( pod ) then return end
	...
	vehicle.sm_ppb_pitch = vehicle.sm_ppb_pitch and math.ApproachAngle( vehicle.sm_ppb_pitch, Angles.p, AimRate * FrameTime() ) or 0
	
	local TargetAng = Angle(vehicle.sm_ppb_pitch,0)
	TargetAng:Normalize() 

	vehicle:SetPoseParameter("body_aim_pitch", -TargetAng.p )
end
...
function simfphys.weapon:ControlBody( vehicle, deltapos )
	...
	self:AimBody( ply, vehicle, pod, Attachment )
end
...
function simfphys.weapon:Attack( vehicle, ply, shootOrigin, Attachment, ID )
	...
	self:SetNextFire( vehicle, CurTime() + 0.07 + (vehicle.smTmpMG ^ 5) * 0.05 )
end
...
function simfphys.weapon:SecondaryAttack( vehicle, ply, deltapos, cPos, cAng )
	...
	local ID = vehicle:LookupAttachment( "can_mg_muzzle" )
	local Attachment = vehicle:GetAttachment( ID )
	...
	self:SetNextSecondaryFire( vehicle, CurTime() + 0.12 + (vehicle.smTmpHMG ^ 5) * 0.05 )
end
...
function simfphys.weapon:Think( vehicle )
	...
	self:ControlBody( vehicle, deltapos )
    ...
    local TrackTurnRate = 28
    local TrackMultRate = 50
...
function simfphys.weapon:UpdateSuspension( vehicle )
	...
	for i, v in pairs( udes03_susdata ) do
		local pos = vehicle:GetAttachment( vehicle:LookupAttachment( udes03_susdata[i].attachment ) ).Pos + Up * 5
		...
		vehicle:SetPoseParameter(udes03_susdata[i].poseparameter, vehicle.oldDist[i] )
	end
end
