...
local function cannon_fire(ply,vehicle,shootOrigin,shootDirection)
	...
	vehicle:GetPhysicsObject():ApplyForceOffset( -shootDirection * 800000, shootOrigin ) 
...
function simfphys.weapon:Initialize( vehicle )
	...
	simfphys.RegisterCamera( vehicle:GetDriverSeat(), Vector(-10,7,30), Vector(-25,40,110), true, "cannon_mg_muzzle" )
	
	if not istable( vehicle.PassengerSeats ) or not istable( vehicle.pSeat ) then return end

	simfphys.RegisterCrosshair( vehicle.pSeat[1] , { Attachment = "mg_muzzle", Type = 5 } )
	simfphys.RegisterCamera( vehicle.pSeat[1], Vector(-50,10,-15), Vector(0,50,110), true, "mg_muzzle" )
...
function simfphys.weapon:AimCannon( ply, vehicle, pod, Attachment )	
	...
	local pclamp = math.Clamp( (math.cos( math.rad(TargetAng.y) ) - 0.7) * 6,0,1) ^ 2 * -4
