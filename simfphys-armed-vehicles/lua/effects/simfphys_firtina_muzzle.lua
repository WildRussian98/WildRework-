...
function EFFECT:Muzzle( pos, dir, vel )
	...
	for i = 0,20 do
		...
		if particle then
			...
			particle:SetStartSize( math.random(204,460) )
...
function EFFECT:Smoke( pos, dir, vel )
	...
	for i = 0,25 do
		...
		if particle then
			particle:SetVelocity( dir * math.Rand(300,1800) + VectorRand() * math.Rand(0,200) + vel )
...
function EFFECT:Think()
	...
	if self.DieTime > CurTime() then
		...
		for i = 0,math.Rand(3,6) do
			...
			if particle then
				particle:SetVelocity( dir * 2 + RandVector() * 30 )
