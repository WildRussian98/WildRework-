...
local function cannon_fire(ply,vehicle,shootOrigin,shootDirection)
	...
	local projectile = {}
		...
		projectile.shootDirection = shootDirection + (VectorRand() * 0.008)
		projectile.attacker = ply
		projectile.attackingent = vehicle
		projectile.Damage = 2500
		projectile.Force = 600
		projectile.Size = 4
...
function simfphys.weapon:Initialize( vehicle )
	net.Start( "avx_ins1_register_tank" )
		...
		net.WriteString( "bmp1" )
	net.Broadcast()

	simfphys.RegisterCrosshair( vehicle:GetDriverSeat(), { Attachment = "muzzle_cannon", Direction = Vector(0, 0, -1), Type = 4 } )
	simfphys.RegisterCamera( vehicle:GetDriverSeat(), Vector(0,-10,50), Vector(0,50,110), true, "muzzle_cannon" )
...
function simfphys.weapon:PrimaryAttack( vehicle, ply, shootOrigin, Attachment )
	...
	self:SetNextPrimaryFire( vehicle, CurTime() + (40 / 13) )
end
...
function simfphys.weapon:Think( vehicle )
	...
	for i, missile in pairs(vehicle.MissileTracking or {}) do
		if IsValid( missile ) then
			...
			local vel = -missile:GetVelocity() + missile.DirVector * 5000
