...
local function cannon_fire(ply,vehicle,shootOrigin,shootDirection)
	...
	vehicle:GetPhysicsObject():ApplyForceOffset( -shootDirection * 25000, shootOrigin )
...
function simfphys.weapon:ValidClasses()
	return { "sim_fphys_m142a1" }
end

function simfphys.weapon:Initialize( vehicle )
	net.Start( "avx_misc_register_tank" )
		net.WriteEntity( vehicle )
		net.WriteString( "m142a1" )
	net.Broadcast()
	
	vehicle:SetNWBool( "SpecialCam_Loader", true )
	vehicle:SetNWFloat( "SpecialCam_LoaderTime", 5 )

	simfphys.RegisterCrosshair( vehicle:GetDriverSeat(), { Attachment = "cannon_muzzle", Direction = Vector(0, 0, -1), Type = 2 } )
	simfphys.RegisterCamera( vehicle:GetDriverSeat(), Vector(0,-37,-20), Vector(0,20,110), true, "atgm_muzzle" )
	
	simfphys.RegisterCamera( vehicle.pSeat[1], Vector(-20,-72,42), Vector(0,20,110), true, "atgm_muzzle" )
...
function simfphys.weapon:ControlTurret( vehicle, deltapos )
	...
	local ID2 = vehicle:LookupAttachment( "atgm_muzzle" )
...
function simfphys.weapon:PrimaryAttack( vehicle, ply, shootOrigin, Attachment )
   ...
   self.GunClip = self.GunClip or self.GunClipsize
	...
	util.Effect( "arctic_m142a1_muzzle", effectdata, true, true )

	self:SetNextPrimaryFire( vehicle, CurTime() + 0.4 )
end
...
function simfphys.weapon:Think( vehicle )
	...
	local ID = vehicle:LookupAttachment( "atgm_muzzle" )
	local Attachment = vehicle:GetAttachment( ID )
	...
	for i, missile in pairs(vehicle.MissileTracking or {}) do
		if IsValid( missile ) then
			...
			local vel = -missile:GetVelocity() + missile.DirVector * 30000
...
function simfphys.weapon:UpdateSuspension( vehicle )
	...
	for i, v in pairs( m142a1_susdata ) do
		local pos = vehicle:GetAttachment( vehicle:LookupAttachment( m142a1_susdata[i].attachment ) ).Pos + Up * 16
		...
		vehicle:SetPoseParameter(m142a1_susdata[i].poseparameter, vehicle.oldDist[i] )
	end
end
