...
local function cannon_fire(ply,vehicle,shootOrigin,shootDirection)
	...
	util.Effect( "arctic_btr_muzzle", effectdata, true, true )
...
function simfphys.weapon:Initialize( vehicle )
	...
	simfphys.RegisterCamera( vehicle.pSeat[1], Vector(0,0,45), Vector(0,0,45), true )	
	simfphys.RegisterCamera( vehicle.pSeat[2], Vector(0,0,45), Vector(0,0,45), true )
	simfphys.RegisterCamera( vehicle.pSeat[3], Vector(0,0,45), Vector(0,0,45), true )	
	simfphys.RegisterCamera( vehicle.pSeat[4], Vector(0,0,45), Vector(0,0,45), true )
	simfphys.RegisterCamera( vehicle.pSeat[5], Vector(0,0,45), Vector(0,0,45), true )
	simfphys.RegisterCamera( vehicle.pSeat[6], Vector(0,0,45), Vector(0,0,45), true )
	
	for i = 2, table.Count( vehicle.pSeat ) do
	simfphys.RegisterCrosshair( vehicle:GetDriverSeat(), { Attachment = "muzzle_2A72", Type = 2 } )
	simfphys.RegisterCamera( vehicle:GetDriverSeat(), Vector(-83,-7,-15), Vector(20,40,60), true, "muzzle_2A72" )
	end
...
function simfphys.weapon:ControlTurret( vehicle, deltapos )
	...
	local ID = vehicle:LookupAttachment( "muzzle_2A72" )
...
function simfphys.weapon:PrimaryAttack( vehicle, ply, shootOrigin, Attachment )
	...
	cannon_fire( ply, vehicle, shootOrigin + shootDirection * 20, shootDirection )
